...
impl BlackHoleShader {
    pub fn new(device: &Device, _queue: &Queue) -> AstrariaResult<Self> {
        ...
        let pipeline_layout = generated_shaders::black_hole::create_pipeline_layout(device);
        ...
        let pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            ...
            vertex: generated_shaders::black_hole::vertex_state(&shader, &vertex_entry),
            fragment: Some(generated_shaders::black_hole::fragment_state(
                &shader,
                &fragment_entry,
            )),
